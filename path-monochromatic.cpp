...
* dominated by v using paths of length at most 2. Let s(Q, c) be the sum of s(Q, c, v) taken over all vertice (As Q is a tournament, s(Q,c) can also be determined by the number of directed triangles
* and the number of monochromatic directed triangles in Q), and let s(Q) be the minimum of s(Q, c) taken over all colorings.
*
...
/* Prerequisites:
*
* 1) A file containing the databse of all tournaments on q vertices. This is taken from https://users.cecs.anu.edu.au/~bdm/data/digraphs.html for q <= 9.
*
*/
...
const int resultFilter = 2 * q * (q - 1) / 3;									// discard tournaments which have a coloring that does not reach as lest this result
...
void setTournamentDB() {
	for (int counter = 0; counter < numTournaments[q]; counter++)  // loop on all tournaments in the tournamnt database.
		nextTournamentFromFile(counter);  // read the next tournament Q from the tournament database file.
}
...
int nextColor(int r) {
	for (int i = 0; i < q; i++)
		for (int j = i + 1; j < q; j++) {
			if (i + j == 1) // this saves half the runtime as the first examined edge can always be fixed some particular color, say color 1.
				continue;
